...
public class CourseTableMaker {
    ...
    public static void loader(List<CourseModel> list) {
        for (int i = 0; i < courseModels.length; i++) {
            courseModels[i].clear();
        }
        for (int i = 0; i < list.size(); i++) {
            CourseModel temp = list.get(i);
            courseModels[temp.getWeek() - 1].add(temp);
        }
    }
...
//        models_2.add(new CourseModel(5, "Java", 9, 2, 6, "A828", (int) (Math.random() * 10)));
